...
function App() {
  ...
  // Lets the static per-boss SEO pages (public/bosses/<id>/, generated by
  // scripts/generate-boss-pages.mjs) deep-link into the real simulator via
  // e.g. /?npc=zulrah, landing the visitor straight on that boss's loot
  // table instead of the boss browser. useGameData() is Suspense-gated (see
  // GameDataContext.tsx), so npcs is already populated on this very first
  // render — no need to wait for it in an effect. Read once per render
  // (cheap, and the URL doesn't change here) rather than memoized, since
  // it's only actually consumed by the two initializers below plus the
  // one-time cleanup effect further down.
  const npcParam = new URLSearchParams(window.location.search).get("npc");
  const npcFromParam = npcParam ? npcs.find((n) => n.id === npcParam) : undefined;
  ...
  // selected last is disorienting as a first screen. Exception: a visitor
  // arriving via a boss's own SEO page already picked a boss, on that page —
  // jump straight to its loot table like clicking it in the browser would.
  const [mobileTab, setMobileTab] = useState<MobileTab>(npcFromParam ? "detail" : "bosses");
  ...
  const [selectedNpc, setSelectedNpc] = useState<Npc | null>(() => {
    if (npcFromParam) return npcFromParam;
  ...
  // selectedNpc/mobileTab above already picked up npcFromParam directly at
  // initializer time — this effect only handles the two side effects that
  // go with a *normal* selection (persisting it, matching handleSelectNpc)
  // that a plain state initializer can't do: writing to localStorage and
  // cleaning ?npc= off the URL so a reload or share of the resulting link
  // doesn't keep re-landing here. Depending on npcFromParam looks like it'd
  // re-fire on every later render (its value flips to undefined the moment
  // the URL cleanup below removes ?npc=, since it's re-derived from the URL
  // each render) — but the guard just no-ops that second firing, so this is
  // still effectively once, just expressed in a way the deps check can verify.
  useEffect(() => {
    if (!npcFromParam) return;
    localStorage.setItem(LAST_NPC_STORAGE_KEY, npcFromParam.id);
